<div class="relative py-[125px] flex items-center" style="background-image: url({{Vite::asset('resources/images/contact_us_bg.jpg')}});">
    <div class="absolute bottom-0 h-[242px] w-[877px] max-w-full bg-no-repeat bg-bottom" style="background-image:url({{Vite::asset('resources/images/breadcrumb_shape.png')}});">
    </div>
    <section class="container">
        <h1 class="text-5xl font-bold text-white">{{$title}}</h1>
    </section>
</div>
